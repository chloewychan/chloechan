...
.photostrip-img {
  ...
  /* `<picture>` with `left: 50%` + `width: auto` isn't treated as a
     definite replaced element, so without an explicit aspect-ratio it
     falls into the CSS shrink-to-fit sizing algorithm and collapses to
     half the containing block's width instead of following the image's
     intrinsic ratio. This ratio is ChloeFriendly's exact photostrip.png
     ratio (1714x4096); Sewciety's is ~0.4202 vs this ~0.4185 — close
     enough (~0.4%) that sharing one approximate value for both is fine. */
